...
namespace frontier_exploration {
        ...
        _search{}
    {
        ...
        _smClient = _nh->serviceClient<std_srvs::Empty>("/state_control/explore/stopped");
    ...
    void Explore::stop()
    {
        ...
        ROS_INFO("Sending message to state machine that we are finished with exploring");
        if (_smClient.exists())
        {
            ROS_INFO("Service server is running");
            std_srvs::Empty srv;
            _smClient.call(srv);
            ROS_INFO("Explore.cpp: State machine finished exploring called successfully!");
        }
        else
        {
            ROS_WARN("Service server is not running, state machine is not notified!");
        }
